...
module.exports = function(app) {
  ...
  app.post('/getresponse', (req, res) => {
    console.log(req.body);
    ResponseModel.find({type: req.body.type}).limit(1).sort('-_id').select('type text readCount').exec((err, response) => {
      if (err) {
        console.log(err)
      }
      ...
      if (response[0].readCount > 0) {
        res.json({
          response: {
            text: "Could you give me a few more seconds? I'm still processing everything that's going on."
          }
        })
      } else {
